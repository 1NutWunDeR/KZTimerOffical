...
public PrintMapRecords(client)
{
	...
	mintime+=0.0002;
	if (g_global_ValidedMap && g_global_IntegratedButtons && !g_global_VersionBlocked && !g_bGlobalBeta && !g_global_SelfBuiltButtons && g_hDbGlobal != INVALID_HANDLE && g_bEnforcer && g_global_ValidFileSize && !g_bAutoTimer)
	{		
		if (g_global_fRecordTime102 != 9999999.0)
		{
			if (g_global_fRecordTime102 <= mintime)
			{
				FormatTimeFloat(client, g_global_fRecordTime102, 3,szTime,sizeof(szTime));
				PrintToChat(client, "%t", "GlobalRecord2",MOSSGREEN,WHITE,RED,WHITE, szTime, g_global_RecordPlayerName102); 
			}
		}	
		if (g_global_fRecordTime64 != 9999999.0)
		{
			if (g_global_fRecordTime64 <= mintime)
			{
				FormatTimeFloat(client, g_global_fRecordTime64, 3,szTime,sizeof(szTime));
				PrintToChat(client, "%t", "GlobalRecord1",MOSSGREEN,WHITE,RED,WHITE, szTime, g_global_RecordPlayerName64);
			}
		}	
		if (g_global_fRecordTime128 != 9999999.0)
		{
			if (g_global_fRecordTime128 <= mintime)
			{
				FormatTimeFloat(client, g_global_fRecordTime128, 3,szTime,sizeof(szTime));
				PrintToChat(client, "%t", "GlobalRecord3",MOSSGREEN,WHITE,RED,WHITE, szTime, g_global_RecordPlayerName128); 
			}
		}	
	}
...
public MacroBan(client)
{
	if (!g_bAutoBhop && !g_bFlagged[client])
	{
		...
		if (g_hDbGlobal != INVALID_HANDLE)
		{
			decl String:szName[64];
			GetClientName(client,szName,64);
			db_InsertBan(g_szSteamID[client], szName, g_szCountryCode[client], reason, globalbanstats);
		}	
...
public RegServerConVars()
{
	...
	g_hGlobalBanList = 	CreateConVar("kz_global_ban_list", "1", "on/off - Kicks players automatically if they are listed on the global banlist of kztimer", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_bGlobalBanList     = GetConVarBool(g_hGlobalBanList);
	HookConVarChange(g_hGlobalBanList, OnSettingChanged);	
...
public RegConsoleCmds()
{
	...
	RegConsoleCmd("sm_mapinfo", Client_Mapinfo, "[KZTimer] prints in chat information about the current map");
