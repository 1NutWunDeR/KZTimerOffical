...
{	
	...
	{
		if (g_Server_Tickrate == 64 && g_fFinalTime[client] < g_global_fRecordTime64)
		{
			g_global_fRecordTime64 = g_fFinalTime[client];
			Format(g_global_RecordPlayerName64, MAX_NAME_LENGTH, "%s", szName);	
			g_FinishingType[client] = 3;
			g_Sound_Type[client] = 1;	
		}
		else
		{
			if (g_Server_Tickrate == 128 && g_fFinalTime[client] < g_global_fRecordTime128)
			{
				g_global_fRecordTime128 = g_fFinalTime[client];
				Format(g_global_RecordPlayerName128, MAX_NAME_LENGTH, "%s", szName);	
				g_FinishingType[client] = 4;
				g_Sound_Type[client] = 1;	
			}
			else
				if (g_Server_Tickrate == 102 && g_fFinalTime[client] < g_global_fRecordTime102)
				{
					g_global_fRecordTime102 = g_fFinalTime[client];
					Format(g_global_RecordPlayerName102, MAX_NAME_LENGTH, "%s", szName);	
					g_FinishingType[client] = 5;
					g_Sound_Type[client] = 1;	
				}
		}	
	}
