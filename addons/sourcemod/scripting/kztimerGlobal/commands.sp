...
public Action:Client_Wr(client, args)
{
	if (IsValidClient(client))
	{
		if (g_global_fRecordTime102 == 9999999.0 && g_global_fRecordTime64 == 9999999.0 && g_global_fRecordTime128 == 9999999.0 && g_fRecordTimePro == 9999999.0 && g_fRecordTime == 9999999.0)
...
public MapTopMenu(client, String:szMap[128])
{
	...
	if (g_global_ValidedMap && !g_global_VersionBlocked && g_hDbGlobal != INVALID_HANDLE && FileSize1)
	{
		if (g_Server_Tickrate==128)
			AddMenuItem(topmenu2, "!globaltop", "Top 20 Global");
		else
	...
	if(action == MenuAction_Select)
	{
		switch(param2)
		{
			...
			case 3: 
			{
				...
				if (g_Server_Tickrate==128)
					db_selectGlobalTopClimbers128(param1,globalmap);
				else
					if (g_Server_Tickrate==102)
						db_selectGlobalTopClimbers102(param1,globalmap);
					else			
						db_selectGlobalTopClimbers(param1,globalmap);
			}
		}
	}
	else
...
public ShowSrvSettings(client)
{
	...
	PrintToConsole(client, "kz_global_ban_list %b", g_bGlobalBanList);
