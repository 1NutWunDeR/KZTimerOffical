...
new Handle:g_hGlobalBanList = INVALID_HANDLE;
new Handle:g_hGlobalBanListArray = INVALID_HANDLE;
new bool:g_bGlobalBanList;
...
new const String:SMX_PATH[] = "plugins/KZTimerGlobal.smx";
...
new g_global_KZTimerFileSize;
new g_global_VersionBlocked;
new g_global_maprank[MAXPLAYERS+1];
...
new Float:g_global_fRecordTime102=9999999.0;
new Float:g_global_fRecordTime64=9999999.0;
new Float:g_global_fRecordTime128=9999999.0;
...
new String:g_global_RecordPlayerName64[MAX_NAME_LENGTH];
new String:g_global_RecordPlayerName102[MAX_NAME_LENGTH];
new String:g_global_RecordPlayerName128[MAX_NAME_LENGTH];
...
{	
	...
	g_bGlobalBeta=false;
	if (StrContains(g_szMapName,"_beta",false) != -1 || StrContains(g_szMapName,"_alpha",false) != -1)
		g_bGlobalBeta=true;
...
{	
	...
	if (g_bGlobalBanList && g_hGlobalBanListArray != INVALID_HANDLE)
	{
		for (new i = 0; i < GetArraySize(g_hGlobalBanListArray); i++)
		{
			...
			TrimString(szAuthID);
			if (StrEqual(szAuthID,g_szSteamID[client]))
			{
...
{	
	...
	if(convar == g_hGlobalBanList)
	{
		if(newValue[0] == '1')		
			g_bGlobalBanList = true;
		else
			g_bGlobalBanList = false;
	}
...
public OnGameFrame()
{
	...
	for (new i = 1; i <= MaxClients; i++) 
	{
		if(IsValidClient(i) && IsPlayerAlive(i) && !PlayerInTriggerPush[i]) 
		{
