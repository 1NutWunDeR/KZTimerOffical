...
public db_createTables()
{
	...
	SQL_FastQuery(g_hDb, "ALTER TABLE playerjumpstats3 ADD cjstrafes INT DEFAULT '0'"); //added in 1.73
	SQL_FastQuery(g_hDb, "ALTER TABLE playerjumpstats3 ADD cjsync INT DEFAULT '0'"); //added in 1.73
	SQL_FastQuery(g_hDb, "ALTER TABLE playerjumpstats3 ADD cjheight FLOAT DEFAULT '-1.0'"); //added in 1.73
...
public SQL_ViewJumpStatsCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	Format(szQuery, 4096, sql_selectJumpStats2, szSteamId);  
...
{       
	...
	if(SQL_HasResultSet(hndl))
	{
		...
		{
			SQL_FetchString(hndl, 0, szName, 64);
			wjrecord = SQL_FetchFloat(hndl, 1); 
			strafes = SQL_FetchInt(hndl, 2); 
			SQL_FetchString(hndl, 3, szSteamID, 32);
			if (strafes < 10)
				Format(szStrafes, 32, " %i ", strafes); 
				...
				Format(szStrafes, 32, "%i", strafes); 
			if (i < 10)
					Format(szValue, 128, "[0%i.]    %.3f units      %s      » %s", i, wjrecord,szStrafes, szName);
				else
					Format(szValue, 128, "[%i.]    %.3f units      %s      » %s", i, wjrecord,szStrafes, szName);
			AddMenuItem(menu, szSteamID, szValue, ITEMDRAW_DEFAULT);
			i++;
		}
	}
...
public db_insertLastPositionCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (1 <= client <= MaxClients)
	{
		...
		new tickrate = g_Server_Tickrate;
